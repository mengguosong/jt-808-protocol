...
//		ret.setLatitude(this.parseFloatFromBytes(data, 8, 4));
		double lati = (double) this.parseIntFromBytes(data, 8, 4);
		double latiResult = lati/1000000L;
		ret.setLatitude(latiResult);
		// 4. byte[12-15] 经度(DWORD(32)) 以度为单位的经度值乘以10^6，精确到百万分之一度
//		ret.setLongitude(this.parseFloatFromBytes(data, 12, 4));
		double longti = (double) this.parseIntFromBytes(data, 12, 4);
		double longtiResult = longti/1000000L;
		ret.setLongitude(longtiResult);
	...
	private float parseFloatFromBytes(byte[] data, int startIndex, int length, float defaultVal) {
		try {
			...
			byte[] tmp = new byte[len];
			System.arraycopy(data, startIndex, tmp, 0, len);
			return bitOperator.byteBE2Float(tmp);
		} catch (Exception e) {
